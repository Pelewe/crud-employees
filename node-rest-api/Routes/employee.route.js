...
const app = express();
const employeeRoute = express.Router('/employees');
...
// To search employee by firstname, lastname or email
employeeRoute.route('/search/:name').get(function (req, res) {
    var nameRegex = { "$regex": new RegExp('^' + req.params.name.toLowerCase(), 'i') };

    //    employeeModel.find({ firstName: nameRegex } || {lastName: nameRegex} || {email: nameRegex}, function(err,   data) {
    //         if (err) throw err;
    //         res.send(data);
    //     });
